      ...
      <div class="header-right">
        <div class="nav-buttons">
          <button
              ...
              @click="scrollBy(isMobile ? -100 : -370)"
          >
      ...
      <div class="scroll-container" ref="scrollContainerRef">
        <div class="events-list">
          <div
              ...
              :key="news.id"
              class="event-card"
              :style="isMobile ? { minWidth: 'calc(100vw - 60px)' } : {}"
...
const updateScrollStatus = () => {
  ...
  // 更新当前指示点
  const cardWidth = isMobile ? (window.innerWidth - 60) : 350; // 手机时卡片宽度为屏幕宽度减去边距
  const gap = 20; // 卡片间距
...
const dots = computed(() => {
  try {
    if (!scrollContainerRef.value || !newsList.value || newsList.value.length === 0) {
      return [];
    }

    const container = scrollContainerRef.value;
    const cardWidth = isMobile.value ? (window.innerWidth - 60) : 350;
    const gap = 20;
    const containerWidth = container.clientWidth;

    if (containerWidth <= 0 || (cardWidth + gap) <= 0) {
      return [];
    }

    const visibleCards = Math.max(1, Math.floor(containerWidth / (cardWidth + gap)));
    const totalDots = Math.max(0, Math.ceil(newsList.value.length / visibleCards));
